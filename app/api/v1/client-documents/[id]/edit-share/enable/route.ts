...
import { generateAccessCode, generateEditShareToken } from '@/lib/client-documents/editShare'
...
export const POST = withAuth('admin', async (req: NextRequest, user: ApiUser, ctx: RouteContext) => {
  ...
  const doc = snap.data() as { orgId?: string; deleted?: boolean; editShareToken?: string } | undefined
  ...
  const editShareToken = doc.editShareToken ?? generateEditShareToken()
  ...
  return apiSuccess({ editShareToken, editAccessCode, editShareEnabled: true })
})
