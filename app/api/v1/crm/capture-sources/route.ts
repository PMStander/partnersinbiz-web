/**
 * GET  /api/v1/crm/capture-sources?orgId=...   — list sources for an org
 * POST /api/v1/crm/capture-sources             — create a new capture source
 *
 * Body (POST): { orgId, name, type, autoTags?, autoCampaignIds?, redirectUrl?, consentRequired? }
 * Auth: admin
 */
...
export const GET = withAuth('client', async (req: NextRequest, user) => {
  ...
  const sources: CaptureSource[] = snap.docs
    ...
    .sort((a: CaptureSource, b: CaptureSource) => {
      const aMs = a.createdAt ? (a.createdAt as { _seconds?: number; seconds?: number })._seconds ?? (a.createdAt as { _seconds?: number; seconds?: number }).seconds ?? 0 : 0
      const bMs = b.createdAt ? (b.createdAt as { _seconds?: number; seconds?: number })._seconds ?? (b.createdAt as { _seconds?: number; seconds?: number }).seconds ?? 0 : 0
      return bMs - aMs
    })
...
export const POST = withAuth('client', async (req: NextRequest, user) => {
  ...
  const docRef = await adminDb.collection('capture_sources').add({
    orgId,
    name,
    type,
