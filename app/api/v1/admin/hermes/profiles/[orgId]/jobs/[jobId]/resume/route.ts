...
export const POST = withAuth('admin', async (req: NextRequest, user, ctx) => {
  ...
  const { response, data } = await callHermesJson(access.link, `/api/jobs/${encodeURIComponent(jobId)}/resume`, {
    method: 'POST',
    ...(body ? { body } : {}),
  })
  if (!response.ok) return apiError(hermesError(data, 'Hermes job resume failed'), response.status || 502, { hermes: data })
