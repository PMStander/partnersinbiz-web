// lib/ads/providers/google/campaigns.ts
// Google Ads Search campaign CRUD helper — Sub-3a Phase 2 Batch 2.
...
/** Create a Search campaign. Returns Google resource name + id. */
    ...
    operations: [
      {
        create: {
          ...
          networkSettings: googleSearchNetworkSettings(),
          manualCpc: { enhancedCpcEnabled: false },  // default MANUAL_CPC
        },
      },
    ],
  }
...
/** Update existing Search campaign — name, status only in MVP. */
...
/** Pause campaign (sets status to PAUSED) */
export async function pauseCampaign(args: CallArgs & { resourceName: string }): Promise<GoogleMutateResult> {
  return updateCampaign({ ...args, status: 'PAUSED' })
}

/** Resume campaign (sets status to ENABLED via canonical ACTIVE) */
export async function resumeCampaign(args: CallArgs & { resourceName: string }): Promise<GoogleMutateResult> {
  return updateCampaign({ ...args, status: 'ACTIVE' })
}

/** Remove campaign (sets status to REMOVED via canonical ARCHIVED) */
export async function removeCampaign(args: CallArgs & { resourceName: string }): Promise<GoogleMutateResult> {
  const body = {
